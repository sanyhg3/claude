...
class CDPController {
  ...
  async clickAt(x, y) {
    if (!launcher.activeCDP) return;
    const cx = Math.round(x), cy = Math.round(y);
    const base = { x: cx, y: cy, button: 'left', clickCount: 1 };
    await launcher.activeCDP.send('Input.dispatchMouseEvent', { type: 'mouseMoved', x: cx, y: cy }).catch(() => {});
    await launcher.activeCDP.send('Input.dispatchMouseEvent', { type: 'mousePressed', ...base }).catch(() => {});
    await launcher.activeCDP.send('Input.dispatchMouseEvent', { type: 'mouseReleased', ...base }).catch(() => {});
  }
