        ...
        try {
          ...
          switch (msg.type) {
            case 'init': {
              ...
              targetSpecs.ua = targetSpecs.ua || 'Mozilla/5.0 (Linux; Android 13; Pixel 7) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Mobile Safari/537.36';

              const isDead = !launcher.isPageReady();
              const specs = sessionManager.currentSpecs;
              const specsChanged = specs.w !== targetSpecs.w || specs.h !== targetSpecs.h || specs.dpr !== targetSpecs.dpr || specs.ua !== targetSpecs.ua;
              ...
              if (isDead) {
                ...
                await launcher.startNativeBrowser(targetSpecs.w, targetSpecs.h, targetSpecs.dpr, targetSpecs.ua, async () => {
                  this.isCapturing = false;
                  if (sessionManager.clients.size > 0) this.ensureCapture();
                });
              } else if (specsChanged) {
                sessionManager.currentSpecs = targetSpecs;
                await launcher.setViewport(targetSpecs.w, targetSpecs.h, targetSpecs.dpr, targetSpecs.ua);
                if (this.h264Encoder) { this.h264Encoder.destroy(); this.h264Encoder = null; }
                ...
                if (launcher.page) {
                  console.log('🔄 Specs changed, refreshing page...');
                  launcher.page.reload().catch(() => {});
                }
              }

              if (!isDead && !ws.hasInitialized && launcher.page) {
                launcher.page.reload().catch(() => {});
              }
            ...
            case 'forward':
              ...
              break;
            case 'tap':
              await controller.tap(msg.x, msg.y);
              this.triggerCapture();
              break;
            case 'scroll':
            ...
            if (pcSpecs) {
              ...
              if (specsChanged) {
                ...
                launcher.setViewport(pcSpecs.w, pcSpecs.h, pcSpecs.dpr, pcSpecs.ua).then(() => {
                  ...
                  if (launcher.page) {
                    console.log('🔄 Specs changed (disconnect), refreshing page...');
                    launcher.page.reload().catch(() => {});
                  }
                }).catch(() => {});
              }
